...
from handler.study_records_handler import StudyRecordsHandler, StudyRecordRes
...
def select_material_from_bookshelf(bookshelf_entries: BookshelfEntriesRes) -> Optional[BookshelfEntriesMaterial]:
    """
    ...
    """
    if bookshelf_entries.bookshelf_entries.in_progress and len(bookshelf_entries.bookshelf_entries.in_progress) > 0:
        return bookshelf_entries.bookshelf_entries.in_progress[0]
    
    if bookshelf_entries.bookshelf_entries.open and len(bookshelf_entries.bookshelf_entries.open) > 0:
        return bookshelf_entries.bookshelf_entries.open[0]
    
    if bookshelf_entries.bookshelf_entries.closed and len(bookshelf_entries.bookshelf_entries.closed) > 0:
